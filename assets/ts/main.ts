...
(() => {
	type Theme = 'auto' | 'latte' | 'frappe' | 'macchiato' | 'mocha';

	function getStoredTheme(): Theme | null {
		const savedTheme = localStorage.getItem('theme');
		if (savedTheme === 'auto' || savedTheme === 'latte' || savedTheme === 'frappe' || savedTheme === 'macchiato' || savedTheme === 'mocha') {
	...
	function setTheme(theme: Theme): void {
		if (theme === 'auto' && window.matchMedia('(prefers-color-scheme: dark)').matches) {
			...
			document.documentElement.setAttribute('data-theme-variant', 'mocha');
		} else {
			document.documentElement.setAttribute('data-bs-theme', theme === 'latte' ? 'light' : 'dark');
	...
	window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => {
		var storedTheme = getStoredTheme();
		if (storedTheme !== 'latte' && storedTheme !== 'frappe' && storedTheme !== 'macchiato' && storedTheme !== 'mocha') {
			setTheme(getPreferredTheme());
		}
	});
